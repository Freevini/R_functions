...
#xx <- data.frame("phosphate" =rep(c("no P dependency","up in low P","up in high P"),2),"color" = rep(levels(scatterplot_petunia_data$phosphate),2) )
#xx$y <-rep(3:1,2)
#xx$x <-c(1,1,1,2,2,2)


#plot(xx$x, xx$y, col=as.character(xx$color), pch=c(19,19,19,15,15,15), cex=3, yaxt="n", xaxt="n", ylab="", xlab="", bty="n", ylim=c(0,5), xlim=c(-1,4), xpd=TRUE)
